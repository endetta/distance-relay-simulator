...
- **Zoom & jendela R–X** — `node tools/plane-zoom.test.js`: `wheelZoomFactor` (±15% per 100 px gulir) + `pinchZoomFactor` (pinch dua-jari: renggangan = zoom in), jendela default dipepetkan ke kartu (×0.82 + clamp no-cut, label tick ber-halo putih), kanvas adaptif (diagram mengisi kartu, toolbar zoom sebagai chip di dalam plot).
