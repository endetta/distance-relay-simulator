...
**Clip separation**: the `clipPath` rect wraps only data content (zones, load wedge, bus markers, fault points); grid, axes and **all labels are drawn OUTSIDE the clip** so tick labels always stay visible at any zoom/pan. Tick labels live in the margins (`padL=38, padR=12, padT=14, padB=26`; axis titles `R (Ω)`/`X (Ω)` sit inside the plot).

**Adaptive canvas (no fixed viewBox anymore)**: `renderPlane` reads the element's actual `clientWidth`/`clientHeight` (fallback `640×470` only when there is no layout) and sets the svg `viewBox` to that same size → internal coordinates are 1:1 with CSS px. `fitPlane()` (desktop-lock mode) sizes `#plane` to fill the whole card above the legend and re-renders only when the dimensions actually change (`svg._dims` guard). The zoom toolbar is a **floating chip cluster inside the plot's top-right corner** (`.plane-card .toolbar` `position:absolute`, `pointer-events:none` on the container so only the buttons grab events; chip radius 12px matches the card) — no reserved band eats diagram height (`padT=14`). The **default window is deliberately wider**: after the ratio-fill both spans are ×1.26 (`WIDE`) so the R axis shows a generous, origin-centred range with even side margins (e.g. tick labels −6..6 on a 720×430 canvas); multiplying both spans keeps the ratio → circles stay round. **Gotcha**: because viewBox == CSS px, the wheel/pan handlers now use raw cursor offsets (`e.clientX-rect.left`) and read pads from `svg._map` — there is no `VBG/VBH2` constant pair to keep in sync anymore. Wheel zoom uses `wheelZoomFactor(deltaY, deltaMode)` — ±15% per 100 px of scroll (trackpad line/page deltas normalised), no longer the old 1.15× per event; **two-finger pinch** arrives as `ctrlKey` wheel events and uses `pinchZoomFactor(deltaY)` — spreading fingers (negative deltaY in Chrome) now zooms IN (sign flipped from the old inverted behaviour), ≈×1.08 per 10 px of spread.
...
- **Mode 'Sekuens trip' tanpa sisa blok hijau**: zona `#formulaOut`/`#sirNote` kini
  ...
  tertinggal kosong.
- **Kanvas R–X: jendela diperlebar, zoom roda/pinch & chip toolbar (sesi ini)**: jendela
  default DIPERLEBAR (kedua span ×1.26 setelah ratio-fill → margin kiri–kanan rata,
  lingkaran tetap bulat; label R −6..6 di kanvas 720×430). Zoom roda `wheelZoomFactor`
  = ±15% per 100 px gulir (garis ≈16 px & halaman ≈400 px dinormalisasi) + helper baru
  `pinchZoomFactor(deltaY)` utk pinch dua-jari (wheel `ctrlKey`): renggangan = zoom IN
  (tanda dibalik dari perilaku lama yg terbalik), ≈×1.08 per 10 px. Toolbar zoom (−/+/Fit)
  = chip melayang DI DALAM plot pojok kanan-atas (radius 12px mengikuti kartu; container
  `pointer-events:none`, `padT` 26→14). `renderPlane` ADAPTIF thd ukuran elemen
  (`clientWidth/Height`, fallback 640×470 utk tes; `viewBox` = ukuran → 1:1 px); `fitPlane()`
  mengisi kartu hingga legenda & render ulang hanya bila ukuran berubah (`svg._dims`).
  Handler wheel/pan memakai offset kursor mentah (tanpa konstanta VBG/VBH2). Diuji
  `tools/plane-zoom.test.js` (23 asersi).
