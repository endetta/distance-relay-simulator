...
**Adaptive canvas (no fixed viewBox anymore)**: `renderPlane` reads the element's actual `clientWidth`/`clientHeight` (fallback `640×470` only when there is no layout) and sets the svg `viewBox` to that same size → internal coordinates are 1:1 with CSS px. `fitPlane()` (desktop-lock mode) sizes `#plane` to fill the whole card above the legend and re-renders only when the dimensions actually change (`svg._dims` guard). The zoom toolbar is a **floating chip cluster inside the plot's top-right corner** (`.plane-card .toolbar` `position:absolute`, `pointer-events:none` on the container so only the buttons grab events; chip radius 12px matches the card) — no reserved band eats diagram height (`padT=10`). The **default window is squeezed to the card**: after the ratio-fill both spans are ×0.82 (`WIDE`) so `k=1` hugs the curves (R−X diagram fills the card; zone-1 circle still fully visible vertically); a **no-cut clamp** then auto-widens the R window at the default fit (and on Fit/Reset) if the *drawn* zones (gz-transformed, incl. rev mirrors, +0.25 Ω margin) would be cropped on the sides — active only while `view` is at its default (k=1, cx/cy at origin), never while panning/zooming. Multiplying both spans keeps the ratio → circles stay round. **Gotcha**: because viewBox == CSS px, the wheel/pan handlers now use raw cursor offsets (`e.clientX-rect.left`) and read pads from `svg._map` — there is no `VBG/VBH2` constant pair to keep in sync anymore. Wheel zoom uses `wheelZoomFactor(deltaY, deltaMode)` — ±15% per 100 px of scroll (trackpad line/page deltas normalised), no longer the old 1.15× per event; **two-finger pinch** arrives as `ctrlKey` wheel events and uses `pinchZoomFactor(deltaY)` — spreading fingers (negative deltaY in Chrome) now zooms IN (sign flipped from the old inverted behaviour), ≈×1.08 per 10 px of spread.
...
- **Kanvas R–X DIPEPETKAN ke kartu, zoom roda/pinch & chip toolbar (sesi ini)**: jendela
  default ×0.82 sesudah ratio-fill (k=1 merapat — diagram mengisi kartu, zone-1 masih
  penuh vertikal; label R −4..4 / X −2..2 di kanvas 720×430) + clamp NO-CUT: di fit
  default (k=1 & view origin) jendela R otomatis melebar bila konten DIGAMBAR (zona via
  gz + margin 0.25 Ω) bakal terpotong kiri/kanan — nonaktif saat pan/zoom. Label tick
  R & X kini ber-halo putih (`paint-order:stroke` + `stroke:var(--surface)` width 3)
  agar terbaca di atas kurva; `padT` 14→10. Zoom roda `wheelZoomFactor` = ±15% per
  100 px gulir (garis ≈16 px & halaman ≈400 px dinormalisasi) + helper `pinchZoomFactor`
  utk pinch dua-jari (wheel `ctrlKey`): renggangan = zoom IN (tanda dibalik dari yg
  terbalik), ≈×1.08 per 10 px. Toolbar zoom (−/+/Fit) = chip melayang DI DALAM plot
  pojok kanan-atas (radius 12px mengikuti kartu; container `pointer-events:none`).
  `renderPlane` ADAPTIF thd ukuran elemen (`clientWidth/Height`, fallback 640×470 utk
  tes; `viewBox` = ukuran → 1:1 px); `fitPlane()` mengisi kartu hingga legenda & render
  ulang hanya bila ukuran berubah (`svg._dims`). Handler wheel/pan memakai offset kursor
  mentah (tanpa konstanta VBG/VBH2). Diuji `tools/plane-zoom.test.js` (26 asersi).
