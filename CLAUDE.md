...
**Clip separation**: the `clipPath` rect wraps only data content (zones, load wedge, bus markers, fault points); grid, axes and **all labels are drawn OUTSIDE the clip** so tick labels always stay visible at any zoom/pan. Tick labels live in the margins (`padL=38, padR=12, padT=26, padB=26`; axis titles `R (Ω)`/`X (Ω)` sit inside the plot).

**Adaptive canvas (no fixed viewBox anymore)**: `renderPlane` reads the element's actual `clientWidth`/`clientHeight` (fallback `640×470` only when there is no layout) and sets the svg `viewBox` to that same size → internal coordinates are 1:1 with CSS px. `fitPlane()` (desktop-lock mode) sizes `#plane` to fill the whole card above the legend and re-renders only when the dimensions actually change (`svg._dims` guard). The zoom toolbar is an **overlay** (`position:absolute` on `.plane-card .toolbar`, `padT=26` headroom so the top labels stay visible). **Gotcha**: because viewBox == CSS px, the wheel/pan handlers now use raw cursor offsets (`e.clientX-rect.left`) and read pads from `svg._map` — there is no `VBG/VBH2` constant pair to keep in sync anymore. Wheel zoom uses `wheelZoomFactor(deltaY, deltaMode)` — ±8% per 100 px of scroll (trackpad line/page deltas normalised) instead of the old 1.15× per event.
...
- **Mode 'Sekuens trip' tanpa sisa blok hijau**: zona `#formulaOut`/`#sirNote` kini
  ...
  tertinggal kosong.
- **Kanvas R–X full-kartu & zoom landai (sesi ini)**: toolbar zoom (−/+/Fit) jadi OVERLAY
  di atas plot (`.plane-card .toolbar` absolut; `padT=26`) — tak ada ruang putih khusus
  yg memakan tinggi. `renderPlane` ADAPTIF thd ukuran elemen (`clientWidth/Height`,
  fallback 640×470 utk tes; `viewBox` diset = ukuran → 1:1 dgn px); `fitPlane()` mengisi
  kartu hingga legenda & render ulang hanya bila ukuran berubah (`svg._dims`). Zoom roda
  memakai `wheelZoomFactor(deltaY,deltaMode)` = ±8% per 100 px gulir (mode garis ≈16 px,
  halaman ≈400 px dinormalisasi) — trackpad tak lagi 1.15× per event. Handler wheel/pan
  memakai offset kursor mentah (tanpa konstanta VBG/VBH2). Diuji `tools/plane-zoom.test.js` (11 asersi).
