...
There is **no build system, package manager, framework, or external tooling** — but
a plain-Node regression suite lives in `tools/` (7 files, 130 assertions, see
*Running it* below). Everything runs in the browser; the only CDN dependency is
Google Fonts.
...
There is no lint/build command; **tests run with plain Node** (commands below). The
JS also runs outside the browser via the mock-DOM harness `tools/lens-harness.js`
(pattern: stub `document`/`window` with elements that capture `innerHTML`, run the
`<script>` body with `new Function`, export `__pub`), then tests regex-measure the
generated `#plane` SVG. This is the pixel-level validation loop for the R–X plane
(label placement, zoom anchor invariance, extreme pan, no-NaN). The seam under test
is the generated SVG string — what the browser actually draws.

In-repo harness & tests: `tools/lens-harness.js` stubs the DOM; the **app publishes the registry**: `const API` at the end of its `<script>` is exposed as `__pub` (names incl. `render,S,P,computeModel,computeFaultCircuit,flowSegments,relayBounds,fitRxWindow,rxWindow,wheelZoomFactor,…`) plus captured elements (`els.<id>.innerHTML`/`textContent`); `tools/readout.test.js` also carries regression scenario S7 (R4 on an L1 fault at pos=30 → `ZONA 3`/`1.20 s`) guarding the `L1−pos` gotcha. `tools/lens.test.js` (run with `node --test tools/lens.test.js`) verifies the **load-encroachment lens**: `loadzMin`/`loadzNom` PRC-023 literals, the **symmetric rounded wedge** (SVG bbox `minX≈−maxX`, ≥6 arc commands = 4 fillets + 2 boundary arcs, `loadRegion` fillet-tangency literals `phiI/phiO/ri1/ri2/rf`, 8 mirrored anchors, degenerate fallback `rf=0` without NaN), and the **dynamic operating point** (`<animateMotion>` closed `M…Z` ellipse path, dur 8–25 s, radial≠tangential `rx≠ry`, and the 4 ellipse cardinal points all staying inside the lens). `tools/readout.test.js` verifies the readout/status card (`.side-card`, mode `'relay'`) — a de-duplicated layout: no `.r-sum` sentence, no `|Z| ∠θ` row, no KaTeX formula (all three repeated the status box / `Z apparent (sekunder)` row / reach rows); the readout is just the two titled groups of `#readout` (`Impedansi gangguan` → `Lokasi gangguan & jangkauan`; the old `Relay & karakteristik` group was trimmed), and `#zoneLabel`/`#timeLabel` texts across trip/behind/no-trip/CT-PT-error scenarios (a no-trip time label distinguishes `di belakang relay` from `di luar seluruh zona`). `tools/sld-v-i.test.js` verifies the SLD V/I feature: `computeFaultCircuit` literals (hand-derived) for a pure-X synthetic network across 3φ/φ-φ/φ-G × infeed off/×1/×4, loop tags, and the always-on SVG chips (the `#viToggle`/`P.showVI` toggle was removed). `tools/flow-anim.test.js` verifies the SLD flow-arrow physics: `flowSegments` segment/colour/`kA` literals plus `#sld` arrow glyphs ∝ `kA` per lane (never copper on L2, nothing past the fault, constant 125 px/s speed). `tools/side-mode.test.js` verifies the trimmed layout & two-mode right card: `tripSequence(m)` order literals, `P.sideMode='seq'` DOM (the status box hidden — its content duplicated the list's first row; `#sirNote` emptied & hidden, no leftover green block; `#formulaOut` element + KaTeX removed from the page), and that `render()` no longer writes `#staircase`/`#typeNote`. `tools/plane-zoom.test.js` verifies the zoom & window spec (`wheelZoomFactor` ±15% per 100 px scroll + `pinchZoomFactor` for two-finger ctrlKey pinch — spread=zoom IN at ≈×1.08 per 10 px, inverted from the old bug; deltaMode normalisation; symmetry), the squeezed-to-card default window (both spans ×0.75, centered on the drawn content with even side gaps, plus a no-cut clamp that auto-widens the R window while at the default fit so drawn zones are never cropped on the sides; isotropic so circles stay round), the white halo on tick labels (`paint-order:stroke` + `stroke:var(--surface)`), the adaptive canvas (`renderPlane` draws at measured `#plane` `clientWidth/Height`; 640×470 fallback only without layout), and the **modul RxMap** — `relayBounds(m)` + `fitRxWindow(view,bounds,canvas)` (murni: rasio isotropik, WIDE 0.75, clamp no-cut @atFit, k-clamp [0.12,64]) + `rxWindow(m)` (fase-1 render, satu-satunya penulis `S.ui.view.cx/cy`, instance → `svg._map`). `tools/title-anim.test.js` verifies the animated header & collapse seams: `.tt-a`/`.tt-b` spans ("Simulator Distance Relay" ↔ "by Sheva - Endetta", crossfade keyframes `ttSwapA/B`, `tt-b` absolute), gradient `--ink→--copper-deep` without `--teal` + left→right shine sweep (`@keyframes ttShine`, band inside `background-size` clipped to text), `prefers-reduced-motion` killing both, the 4 `.card-b-i` wrappers (params cards only — the modal's `.card-b` stays unwrapped) with `grid-template-rows 1fr→0fr` + fade collapse (no `display:none`), and `syncCollapsedCentering()` toggling `.params-panel.all-collapsed` (stack centered when every card is collapsed). Run with `node tools/readout.test.js` / `node tools/sld-v-i.test.js` / `node tools/flow-anim.test.js` / `node tools/side-mode.test.js` / `node tools/plane-zoom.test.js` / `node tools/title-anim.test.js` / `node --test tools/lens.test.js`. All hard-code the HTML filename in their `fs.readFileSync`/`HTML` path — update if the file is ever renamed.
...
4. **Core electrical model** — `computeModel()` is the single source of truth for all derived values. The model is **relay-centric** (4 relays R1–R4), each with a `bus` (A/B/C), a `direction` (`'fwd'`|`'rev'`), a `lineRef` (`'L1'`|`'L2'`), and per-relay reach/timing under `relay.z1Reach`/`z2Reach`/`t2`/`t3`/`rcaOffset`/`qrm`/`charType`. Key conventions to know before editing anything:
   ...
   - **Gotcha `relayFaultZ` Bus C (R4)**: `pos` is ALWAYS measured from source A (Bus A=0, along A→B→C). For a relay at bus B or C looking back along L1, the L1-side distance to a fault at `pos` is **`L1−pos`** (from bus B), never `pos`. R4's L1-fault branch used `seg(pos,…)` → under-measured (e.g. pos=30 read 3.24 Ω → wrong ZONE 2; correct `L1−pos` → 4.42 Ω → ZONE 3). Fixed + regression scenario S7 in `readout.test.js` — do not regress.
...
6. **Rendering** — four independent pure functions that each build an SVG string from `m` (model) and `dec` (decision) and set `innerHTML`:    - `renderPlane(m, dec)` — the R–X diagram (see *Rx diagram notes* below).    - `renderSLD(m)` — buses are thick vertical lines with relay boxes floating beside them (no arrows, no drop lines); a grid/infeed symbol below Bus B toggles `P.infeedOn` and stays in sync with its checkbox. The fault handle drags along the line (updates `S.pos`). All text is collected into the `lbl` string and appended LAST, each label carrying a paint-order halo — labels must never be buried under channel lines, fault arrows, or flow dashes (a past bug). The flow animation is ALWAYS drawn (no toggle) and re-renders with the fault position: lanes come from the pure `flowSegments(m, vi)` — L1 faults: red Sumber→fault (`ia`), copper B→fault (infeed `ib`, only when infeed on); L2 faults: red Sumber→B carries `ia` ONLY (never the infeed — a past artifact made it balloon at Bus B), B→fault is the combined red lane (`ifK`). Arrow glyph size is continuous ∝ `kA` (reference = bolted-at-A current `E/|ZsA|`), speed constant 125 px/s, nothing flows past the fault.
   ...
   - `updateReadout(m, dec)` — two-mode right card driven by `P.sideMode` (default `'relay'`): `'relay'` = the selected relay's status + readout with TWO titled groups (`Impedansi gangguan`, `Lokasi gangguan & jangkauan`) + SIR note. De-duplicated: NO `.r-sum` summary, NO `|Z| ∠θ` row, NO KaTeX formula — all three repeated status/table numbers (KaTeX `#formulaOut` removed from the page); a no-trip time label distinguishes `di belakang relay` from `di luar seluruh zona`. `'seq'` = trip sequence from pure `tripSequence(m)` — enabled relays sorted by operating time (zone-1 instant first, ties by zone then id) then non-trip relays with reason (`di belakang relay` / `di luar jangkauan`); the status box is hidden ENTIRELY (its content = the list's first row — a past duplicate) and `#sirNote` is emptied & hidden (no leftover green block). The staircase card (`renderStaircase`), the `#typeNote`/`typeNotes` block, and the formula box (`#formulaOut`) were removed.
...
7. **Master** `render()` — `computeModel()` → `decideRelay()` per relay → **fase-1 window** `rxWindow(m)` (modul RxMap; satu-satunya penulis `S.ui.view.cx/cy`, instance disalin ke `svg._map`) → **fase-2 gambar** `renderPlane(m,decisions,win)` (murni terhadap view) → `renderSLD` → `updateReadout`. This is the only entry point; every state change funnels through it.
...
- **Kartu readout DE-DUPLIKASI** (`updateReadout`): tanpa kalimat ringkasan `.r-sum`,
  tanpa baris `|Z| ∠θ`, tanpa rumus KaTeX (`#formulaOut` + CDN KaTeX DIHAPUS dari
  halaman) — ketiganya mengulang status box / baris `Z apparent (sekunder)` / tabel
  reach. Kartu = status box (hasil trip; utk TIDAK TRIP baris waktunya membedakan
  `di belakang relay` vs `di luar seluruh zona`) + tabel 2 grup (`Impedansi gangguan` /
  `Lokasi gangguan & jangkauan`; grup `Relay & karakteristik` DIHAPUS). Struktur diuji
  `tools/readout.test.js` — label lama (`r-sum`, `|Z| ∠θ`, `Zone 1 reach`, dst.)
  dijamin tidak hadir.
...
- **Mode 'Sekuens trip' tanpa duplikat & tanpa sisa blok hijau**: kotak status kini
  DISEMBUNYIKAN penuh (isinya dobel dgn baris pertama daftar urutan — relay/zona/waktu
  pertama); catatan `#sirNote` disembunyikan (display none + background di-clear — dulu
  background hijau SIR tertinggal kosong).
- **Kanvas R–X DIPEPETKAN ke kartu, zoom roda/pinch & chip toolbar (sesi ini)**: jendela
  ...
  VBG/VBH2). Jendela kini dihitung modul **RxMap** (`relayBounds`/`fitRxWindow` murni +
  `rxWindow` fase-1) — handler wheel/pan & renderPlane memakai instance `svg._map`
  (`.toPx/.pxToData`); jangan menulis ulang transformasi di handler/renderer. Diuji
  `tools/plane-zoom.test.js` (35 asersi).
- **Kartu kanan dua mode & pemangkasan layout (sesi ini)**: `#sideModeGroup`/`P.sideMode`
  ...
  operasi → zona → id (Z1 seketika), non-trip disertakan dgn alasan; kotak status
  DISEMBUNYIKAN (dobel dgn baris pertama daftar); `#sirNote` dikosongkan. Kartu
  `#staircase` (waktu–impedansi), `#typeNote`/`typeNotes` & kotak rumus `#formulaOut`
  DIHAPUS — kolom kanan berakhir di dasar `plane-row`, jadi batas bawah scroll panel
  parameter = dasar diagram R–X (lock tinggi `.main` otomatis).
...
- **Harness & tes di repo**: `tools/lens-harness.js`, `tools/readout.test.js`,
  `tools/sld-v-i.test.js` (lihat bagian "Running it"). Registry ekspor hidup di aplikasi:
  `const API` di akhir `<script>` (harness cukup menambahkan `;global.__pub=API;`).
  Fungsi baru yang perlu diuji cukup ditambahkan ke `const API` itu (dekat definisinya) —
  jangan daftarkan nama di harness (dua pemilik kebenaran).
