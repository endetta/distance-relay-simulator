...
There is **no build system, no package manager, no test suite, no framework and no external tooling**. Everything runs in the browser, and the only external dependency is loaded from a CDN (Google Fonts).
